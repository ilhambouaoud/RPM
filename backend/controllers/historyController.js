...
exports.getNormalHistory = async (req, res) => {
  ...
  try {
    ...
    const sessions = await ModeNormale.find({
      ...
      portique_id: req.params.portiqueId
    ...
    res.status(500).json({
      error:
        "Erreur récupération historique normal"
    });
...
exports.getBalayageHistory = async (req, res) => {
  ...
  try {
    ...
    const sessions =
      await ModeBalayage.find({

        portique_id:
          req.params.portiqueId
